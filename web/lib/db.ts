    ...
    `WITH active_round AS (
     ...
     alltime AS (
       SELECT s.btc_address,
         ...
         MAX(s.true_difficulty)::TEXT AS best_share
       FROM shares s
       LEFT JOIN session_starts ss ON s.btc_address = ss.btc_address
       WHERE s.round_id = (SELECT id FROM active_round)
...
export function getBlocks(limit = 20, offset = 0) { return _getBlocks(limit, offset) }
